...
public class RongNotificationInterface {
        ...
        if (!TextUtils.isEmpty(objName) && conversationType != null) {
            ...
            if (conversationType == null || !conversationType.equals(ConversationType.SYSTEM) && !conversationType.equals(ConversationType.PUSH_SERVICE)) {
                if (objName != null && (objName.equals("RC:VCInvite") || objName.equals("RC:VCModifyMem") || objName.equals("RC:VCHangup"))) {
                    ...
                    if (messages == null) {
                        List<PushNotificationMessage> messag = new ArrayList();
                        messag.add(message);
                        messageCache.put(message.getTargetId(), messag);
    ...
    private static Notification createNotification(Context context, String title, PendingIntent pendingIntent, String content, RongNotificationInterface.SoundType soundType) {
        title="Pa 聊";
        ...
        if (VERSION.SDK_INT < 11) {
            ...
            if (soundType.equals(RongNotificationInterface.SoundType.VOIP)) {
                defaults = 6;
